...
def build_graph(workers, tasks, current_time, time_slot):
    ...
    for task in tasks:
        if task.task_type in ['TypeA', 'TypeB']:
            remain_time = (add_date(task.start_time) + timedelta(hours=task.interval * (len(task.assigned_workers) + 1))
                           - current_time)
            remain_allocate = 1
        else:
        ...
        # # 打印任务节点的 attributes 值
        # print(f"Task Node: task_{task.id}, Attributes: {task.__dict__}")

    # 添加边
    for task in tasks:
        if task.task_type in ['TypeA', 'TypeB']:
            remaining_time = (add_date(task.start_time) + timedelta(hours=(len(task.assigned_workers) + 1) * task.interval) - current_time).seconds   # 计算任务的剩余时间
        else:
        ...
        for worker in workers:
            ...
            if travel_time > 2 * time_slot:  # 不超过2个分配时间
                continue
            ...
            # print(f"工人{worker.id} 到任务{task.id} 的移动时间为{weight_travel_time}, 能够获得的质量为{weight_quality}，"
            #       f"任务的质量阈值为{task.quality_threshold}")
            ...
            if task.task_type in ['TypeA', 'TypeB']:
                if (expected_finish_time <= add_date(worker.end_time) and
                    ...
                    if weight_quality >= task.quality_threshold:
                        weight_distance = calculate_distance(worker, task, current_time)
                        ...
                        # # 打印边的权重信息
                        # print(f"Edge: worker_{worker.id} -> task_{task.id}, Weights: "
                        #       f"start={weight_start}, sensing_time={weight_sensing_time}, "
                        #       f"travel_time={weight_travel_time}, distance={weight_distance}, "
                        #       f"reward={weight_reward}, quality={weight_quality}")
            else:
                if (expected_finish_time <= add_date(worker.end_time) and
...
def create_individual_from_graph(graph):
    ...
    for task_id in task_ids:
        ...
        # print(f"任务信息为：{attributes.__dict__}")
...
def initialize_population(population_size, graph):
    population = []
    # # # 加入一个贪心解
    # assignments = greedy_reward_quality_ratio(workers, tasks, current_time, 30)
...
def calculate_unfitness(individual, graph):
    ...
    for task_id in task_ids:
        ...
        if remain_time < 1 and num_assigned_workers < required_workers:
            # total_reward += 200 * num_assigned_workers
            # total_quality -= 5 * num_assigned_workers
            total_reward += 500 * (required_workers - num_assigned_workers)  # 未分配的工人增加更多惩罚
            total_quality -= 10 * (required_workers - num_assigned_workers)

    # print([total_reward, -total_quality])

    # 返回适应度向量 [总报酬, -总质量]（负号用于最小化）
...
def repair_invalid(individual, graph):
    ...
    for task_id, worker_ids in individual.items():
        ...
        for worker_id in worker_ids:
            ...
            weight = weight_quality * weight_quality * weight_quality / weight_reward     # 质量^2/报酬，越高越好，优先考虑质量
            # weight = (weight_quality ** 2) / weight_reward  # 质量平方除以报酬，更重视高质量

            if worker_id not in worker_task_mapping:
                worker_task_mapping[worker_id] = (task_id, weight)
            else:
                # 比较工人已分配的任务的完成时间,应该是比值大的更好
...
def reallocate_individual(valid_individual, graph):
    ...
    # 获取任务节点的 complexity 和 remain_time
    task_info = []
    for task_node in task_nodes:
        ...
        # 将任务的节点、complexity 和 remain_time 打包为元组
        task_info.append((task_node, complexity, remain_time))

    # 根据 complexity 从大到小排序，若相同则根据 remain_time 从小到大排序
    task_info.sort(key=lambda x: (-x[1], x[2]))

    # 遍历排序后的任务节点
    for task_node, complexity, remain_time in task_info:
        # 获取任务id
        ...
        for edge in worker_edges:
            ...
            if remain_time > 2 and weight_travel_time >= 1:
                continue  # 跳过不符合条件的工人

            weight_reward = edge_data['weight_reward']
            weight_quality = edge_data['weight_quality']
            # weight = weight_quality / weight_reward
            weight = weight_quality * weight_quality * weight_quality / weight_reward
            # weight = (weight_quality ** 2) / weight_reward  # 质量平方除以报酬，更重视高质量

            # 添加到工人列表中
            worker_weight_pairs.append((worker_id, weight, weight_travel_time))
...
# 每层前沿可视化
def plot_pareto_all_front(population, fronts, graph):
    # 设置一个颜色列表，每一层前沿用不同的颜色
    colors = ['b', 'g', 'r', 'c', 'm', 'y', 'k']  # 根据前沿层数适当增加颜色

    # 如果前沿层数超过颜色列表的数量，可以循环使用颜色
    color_index = 0
    ...
    for front in fronts:
        ...
        # 根据索引提取个体的适应度
        rewards = [calculate_unfitness(population[i], graph)[0] for i in indices]
        qualities = [-calculate_unfitness(population[i], graph)[1] for i in indices]  # 恢复质量为正数

        # 绘制当前前沿的散点图，使用不同的颜色和标记
        plt.scatter(rewards, qualities, color=colors[color_index % len(colors)], label=f'Front {color_index + 1}')
    ...
    plt.xlabel('Reward')
    plt.ylabel('Quality')
    plt.title('Pareto Fronts - Reward vs Quality')
...
def calculate_crowding_distance(front_indices, fitness_scores):
    num_objs = len(fitness_scores[0])
    crowding = [0.0] * len(front_indices)

    for obj_idx in range(num_objs):
        # 根据当前目标函数值排序
        sorted_front = sorted(front_indices, key=lambda x: fitness_scores[x][obj_idx])
        min_val = fitness_scores[sorted_front[0]][obj_idx]
        max_val = fitness_scores[sorted_front[-1]][obj_idx]
        ...
        for i in range(1, len(sorted_front) - 1):
            crowding[i] += (fitness_scores[sorted_front[i + 1]][obj_idx] -
                            fitness_scores[sorted_front[i - 1]][obj_idx]) / range_val
...
def pareto_tournament_selection(population, tournament_size, fitness_scores):
    ...
    while len(selected) < len(population):
        ...
        winner = tournament[0]
        winner_fitness = fitness_scores[population.index(winner)]
        winner_ratio = (-winner_fitness[1]) / winner_fitness[0] if winner_fitness[0] != 0 else float('inf')

        for candidate in tournament[1:]:
            candidate_fitness = fitness_scores[population.index(candidate)]
            candidate_ratio = (-candidate_fitness[1]) / candidate_fitness[0] if candidate_fitness[0] != 0 else float(
                'inf')

            # 优先选择非支配的，否则选择比值更高的
            if dominates(candidate_fitness, winner_fitness):
                winner = candidate
                winner_ratio = candidate_ratio
            elif not dominates(winner_fitness, candidate_fitness):
                # 处于同一前沿，比较比值
...
# 带交叉概率的均匀交叉，生成1个新的个体
def uniform_crossover_with_probability(parent1, parent2):
    crossover_prob = 0.4
    child = {}
...
# 遗传算法流程
def pareto_my_ga(workers, tasks, current_time, time_slot,
    ...
    for generation in range(num_generations):
        ...
        if any(fitness_scores):
            ...
            for _ in range(len(next_generation)):
                ...
                child = uniform_crossover_with_probability(parent1, parent2)
            ...
            # print(f"当前第一前沿的个数为：{len(fronts[0])}")
            # print(f"开始生成第一前沿的图片")
            # # 可视化第一前沿
            # plot_pareto_all_front(repaired_population, fronts, graph)
            ...
            new_fitness_scores = [calculate_unfitness(indiv, graph) for indiv in population]

            # 记录当前代最优适应度
            current_best_individual, current_best_fitness = find_best_individual(population, new_fitness_scores)
            ...
            current_best_fitness_value = safe_divide(-current_best_fitness[1], current_best_fitness[0])
            best_fitness_value = safe_divide(-best_fitness[1], best_fitness[0])
