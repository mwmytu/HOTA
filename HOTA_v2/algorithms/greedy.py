"""
按报价进行贪心，低报酬优先
1.按照任务剩余时间，时间短的优先分配；2.不对移动时间有限制；3.满足质量约束的才能分配
"""
from utils import *
...
def greedy_bid_assign(workers, tasks, current_time, time_slot):
    ...
    tasks.sort(key=lambda x:
        (add_date(x.start_time) + timedelta(hours=(len(x.assigned_workers) + 1) * x.interval) - current_time).seconds
        if x.task_type in ['TypeA', 'TypeB']
        else (add_date(x.end_time) - current_time).seconds)
            ...
            if task.task_type not in ['TypeA', 'TypeB']:
                if (expected_finish_time <= add_date(worker.end_time) and
                    ...
                    if sensing_quality >= task.quality_threshold:
                        ...
                        reward = calculate_pair_sensing_reward(worker, task, current_time)
                        candidate_workers.append((worker, travel_time, total_time, reward))
            else:
                if (expected_finish_time <= add_date(worker.end_time) and
                        expected_finish_time <= (add_date(task.start_time) + timedelta(
