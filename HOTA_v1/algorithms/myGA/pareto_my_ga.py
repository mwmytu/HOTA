...
# 要改，因为是双目标
def create_individual_from_graph_ORIGINAL(graph):
    # 获取所有任务节点的ID
...
def reallocate_individual(valid_individual, graph):
    ...
    for task_node, complexity, remain_time in task_info:
        ...
        for edge in worker_edges:
            ...
            # weight = weight_quality / weight_reward
            weight = weight_quality * weight_quality / weight_reward
...
# 计算拥挤度距离
def calculate_crowding_distance(front, fitness_scores):
    crowding_distances = np.zeros(len(front))
    num_objectives = len(fitness_scores[0])  # 目标的数量

    # 对每个目标（Objective）进行排序并计算拥挤度
    for objective_idx in range(num_objectives):
        sorted_indices = np.argsort([fitness_scores[i][objective_idx] for i in front])
        sorted_front = [front[i] for i in sorted_indices]

        # 边界个体的拥挤度设为无穷大
        crowding_distances[front.index(sorted_front[0])] = float('inf')
        crowding_distances[front.index(sorted_front[-1])] = float('inf')

        for i in range(1, len(sorted_front) - 1):
            distance = fitness_scores[sorted_front[i + 1]][objective_idx] - fitness_scores[sorted_front[i - 1]][
                objective_idx]
            crowding_distances[front.index(sorted_front[i])] += distance
...
def pareto_tournament_selection(population, tournament_size, fitness_scores):
    ...
    while len(selected) < len(population):
        ...
        # ]
        # 获取锦标赛个体的拥挤度
        crowding_distances = calculate_crowding_distance(tournament_indices, fitness_scores)

        # 选择锦标赛中的一个个体
        winner = tournament[0]
        winner_crowding = crowding_distances[0]

        for i in range(1, len(tournament)):
            if dominates(fitness_scores[population.index(winner)], fitness_scores[population.index(tournament[i])]):
                continue
            elif dominates(fitness_scores[population.index(tournament[i])], fitness_scores[population.index(winner)]):
                winner = tournament[i]
                winner_crowding = crowding_distances[i]
            else:
                if crowding_distances[i] > winner_crowding:
                    winner = tournament[i]
                    winner_crowding = crowding_distances[i]
...
# 选择父代中权重较好的
def weighted_crossover_with_probability(parent1, parent2, graph):
    crossover_prob = 0.8
    child = {}
    ...
    for task_id in parent1:
        if random.random() <= crossover_prob:
            ...
            for worker_id in combined_worker_ids:
                worker_node = f"worker_{worker_id}"
                task_node = f"task_{task_id}"
                ...
                if edge is None:
                    continue  # 如果没有连接该工人和任务，则跳过

                weight_reward = edge['weight_reward']
                weight_quality = edge['weight_quality']

                # 计算工人的综合权重：质量^2/报酬
                weight = (weight_quality ** 2) / weight_reward

                # 将工人的ID与权重存储
                worker_weights.append((worker_id, weight))

            # 根据权重进行排序，权重高排在前面
            worker_weights.sort(key=lambda x: x[1], reverse=True)

            # 选择工人的数量是任务要求工人数与去重后工人数的较小值
            required_worker_count = graph.nodes[f"task_{task_id}"]['remain_allocate']
            num_workers_to_assign = min(required_worker_count, len(worker_weights))

            # 从排序后的工人中选出所需数量的工人
            selected_workers = [worker_id for worker_id, _ in worker_weights[:num_workers_to_assign]]

            # 将选中的工人加入子代
            child[task_id] = selected_workers
        else:
            child[task_id] = parent1.get(task_id, [])
...
def random_crossover_with_probability(parent1, parent2, graph):
    crossover_prob = 0.4
    child = {}
    ...
    for task_id in parent1:
        if random.random() <= crossover_prob:
            ...
            # 随机选择所需数量的工人（如果可选工人数量小于任务需求数量，则选择所有工人）
            num_workers_to_assign = min(required_worker_count, len(combined_worker_ids))

            # 随机从工人列表中挑选所需数量的工人
            selected_workers = random.sample(combined_worker_ids, num_workers_to_assign)
