...
def calculate_unfitness(individual, graph):
    ...
    for task_id in task_ids:
        ...
        #     total_reward += 100 * num_assigned_workers
        #     total_quality -= 1 * num_assigned_workers
...
def no_pareto_tournament_selection(population, tournament_size, fitness_scores):
    # 进行非支配排序
...
# 遗传算法流程
def pareto_my_ga_no_front(workers, tasks, current_time, time_slot, population_size, num_generations,
    ...
    for generation in range(num_generations):
        ...
        if any(fitness_scores):
            # 选择适应度较高的个体
            next_generation = no_pareto_tournament_selection(population, tournament_size, fitness_scores)
